...
namespace Resortify.Forms
{
    public partial class CheckoutForm : Form
    {
        ...
        public CheckoutForm() { InitializeComponent(); LoadCart(); }
        ...
        private void InitializeComponent()
        {
            ...
            var summary = new Panel { Dock = DockStyle.Fill, BackColor = Color.White, Padding = new Padding(14), BorderStyle = BorderStyle.FixedSingle };
            var title = new Label { Text = "Booking Summary", Dock = DockStyle.Top, Height = 36, Font = new Font("Segoe UI", 12F, FontStyle.Bold), ForeColor = UIHelper.NavyHeader };
            lstSummary = new ListBox { Dock = DockStyle.Fill, Font = new Font("Segoe UI", 9F), IntegralHeight = false, HorizontalScrollbar = true, BorderStyle = BorderStyle.None };
            summary.Controls.Add(lstSummary); summary.Controls.Add(title);
            ...
            payment.Controls.Add(new Label { Text = "Payment & Discount", Dock = DockStyle.Fill, Font = new Font("Segoe UI", 12F, FontStyle.Bold), ForeColor = UIHelper.NavyHeader, TextAlign = ContentAlignment.MiddleLeft }, 0, 0);

            cboPayment = new ComboBox { Dock = DockStyle.Fill, DropDownStyle = ComboBoxStyle.DropDownList, Margin = new Padding(0, 4, 0, 4) };
            cboPayment.Items.AddRange(new object[] { "Pay at Hotel", "Credit Card", "Debit Card", "bKash", "Nagad", "Rocket", "Mobile Banking" });
            cboPayment.SelectedIndex = 0;
            cboPayment.SelectedIndexChanged += (s, e) => UpdatePaymentFields();
            payment.Controls.Add(cboPayment, 0, 1);

            lblTransaction = new Label { Dock = DockStyle.Fill, Font = new Font("Segoe UI", 9F, FontStyle.Bold), TextAlign = ContentAlignment.MiddleLeft };
            payment.Controls.Add(lblTransaction, 0, 2);

            txtTransaction = new TextBox { Dock = DockStyle.Fill, Margin = new Padding(0, 3, 0, 3), PlaceholderText = "Enter transaction ID" };
            ...
            var couponPanel = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 3, RowCount = 2, Margin = new Padding(0, 2, 0, 2) };
            ...
            txtCoupon = new TextBox { Dock = DockStyle.Fill, Margin = new Padding(0, 4, 6, 4), PlaceholderText = "Coupon code" };
            btnApplyCoupon = UIHelper.MakeButton("Apply", UIHelper.CustomerColor, 80, 30);
            btnApplyCoupon.Dock = DockStyle.Fill; btnApplyCoupon.Margin = new Padding(0, 4, 6, 4); btnApplyCoupon.Click += (s, e) => ApplyCoupon();
            btnRemoveCoupon = UIHelper.MakeButton("Remove", Color.FromArgb(95, 95, 95), 70, 30);
            btnRemoveCoupon.Dock = DockStyle.Fill; btnRemoveCoupon.Margin = new Padding(0, 4, 0, 4); btnRemoveCoupon.Click += (s, e) => RemoveCoupon();
            lblCouponStatus = new Label { Text = "Have a coupon? Try WELCOME10, RESORT15 or GETAWAY20.", Dock = DockStyle.Fill, ForeColor = Color.DimGray, TextAlign = ContentAlignment.MiddleLeft, AutoEllipsis = true };
            couponPanel.Controls.Add(txtCoupon, 0, 0); couponPanel.Controls.Add(btnApplyCoupon, 1, 0); couponPanel.Controls.Add(btnRemoveCoupon, 2, 0);
            couponPanel.Controls.Add(lblCouponStatus, 0, 1); couponPanel.SetColumnSpan(lblCouponStatus, 3);
            payment.Controls.Add(couponPanel, 0, 5);

            var totals = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 2, RowCount = 4, Margin = new Padding(0, 5, 0, 3), Padding = new Padding(0, 4, 0, 4) };
            totals.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 62)); totals.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 38));
            totals.Controls.Add(new Label { Text = "Room subtotal", Dock = DockStyle.Fill, TextAlign = ContentAlignment.MiddleLeft }, 0, 0); lblRoomTotal = Amount(); totals.Controls.Add(lblRoomTotal, 1, 0);
            totals.Controls.Add(new Label { Text = "Services / extras", Dock = DockStyle.Fill, TextAlign = ContentAlignment.MiddleLeft }, 0, 1); lblServiceTotal = Amount(); totals.Controls.Add(lblServiceTotal, 1, 1);
            totals.Controls.Add(new Label { Text = "Total discount", Dock = DockStyle.Fill, TextAlign = ContentAlignment.MiddleLeft }, 0, 2); lblDiscountTotal = Amount(); totals.Controls.Add(lblDiscountTotal, 1, 2);
            totals.Controls.Add(new Label { Text = "Grand total", Dock = DockStyle.Fill, TextAlign = ContentAlignment.MiddleLeft, Font = new Font("Segoe UI", 10F, FontStyle.Bold) }, 0, 3); lblGrandTotal = Amount(true); totals.Controls.Add(lblGrandTotal, 1, 3);
            payment.Controls.Add(totals, 0, 6);

            lblError = UIHelper.MakeErrorLabel(); lblError.Dock = DockStyle.Fill; lblError.AutoSize = false; lblError.TextAlign = ContentAlignment.MiddleLeft; payment.Controls.Add(lblError, 0, 7);
            btnConfirm = UIHelper.MakeButton("Confirm Booking", UIHelper.CustomerColor, 230, 40); btnConfirm.Anchor = AnchorStyles.Right; btnConfirm.Click += BtnPay_Click; payment.Controls.Add(btnConfirm, 0, 8);
            root.Controls.Add(payment, 1, 0);

            bottomPanel = new Panel { Dock = DockStyle.Bottom, Height = 0, Visible = false };
        ...
        private Label Amount(bool strong = false) => new Label { Dock = DockStyle.Fill, TextAlign = ContentAlignment.MiddleRight, Font = new Font("Segoe UI", strong ? 14F : 10F, FontStyle.Bold), ForeColor = UIHelper.CustomerColor };
        ...
        private void LoadCart()
        {
            ...
            lines.Clear(); lstSummary.Items.Clear();
            foreach (DataRow r in table.Rows)
            {
                ...
                var line = new CartLine { CartId = Convert.ToInt32(r["CartId"]), RoomId = Convert.ToInt32(r["RoomId"]), HotelName = r["HotelName"].ToString(), City = r["City"].ToString(), RoomType = r["RoomType"].ToString(), CheckIn = Convert.ToDateTime(r["CheckInDate"]), CheckOut = Convert.ToDateTime(r["CheckOutDate"]), Nights = nights, Quantity = qty, Guests = Convert.ToInt32(r["Guests"]), Services = r["Services"].ToString(), UnitPrice = unit, RoomSubtotal = room, ServiceAmount = service, DiscountAmount = discount };
                lines.Add(line);
        ...
        {
            appliedCouponCode = ""; couponDiscount = 0m;
            if (lines.Count == 0) { lblCouponStatus.Text = "Enter a coupon code if you have one."; return; }
        ...
        private decimal BaseAfterOffers()
        {
            ...
            foreach (var l in lines) { room += l.RoomSubtotal; services += l.ServiceAmount; offers += l.DiscountAmount; }
        ...
        private void RecalculateTotals()
        {
            ...
            foreach (var l in lines) { roomTotal += l.RoomSubtotal; serviceTotal += l.ServiceAmount; offerDiscount += l.DiscountAmount; }
        ...
        private void ApplyCoupon()
        {
            ...
            if (lines.Count == 0) { ShowError("Your cart is empty."); return; }
            string code = txtCoupon.Text.Trim().ToUpperInvariant();
            if (string.IsNullOrWhiteSpace(code)) { ShowError("Enter a coupon code."); return; }
            ...
            if (t.Rows.Count == 0) { RemoveCoupon(false); ShowError("Invalid coupon code."); return; }
            DataRow r = t.Rows[0];
            DateTime today = DateTime.Today;
            if (!Convert.ToBoolean(r["Active"]) || today < Convert.ToDateTime(r["ValidFrom"]).Date || today > Convert.ToDateTime(r["ValidTo"]).Date) { ShowError("This coupon is not currently valid."); return; }
            if (r["MaxUses"] != DBNull.Value && Convert.ToInt32(r["UsedCount"]) >= Convert.ToInt32(r["MaxUses"])) { ShowError("This coupon has reached its usage limit."); return; }
            decimal minimum = Convert.ToDecimal(r["MinimumBookingAmount"]);
            if (baseTotal < minimum) { ShowError($"This coupon requires a minimum booking of ${minimum:N2}."); return; }
            ...
            appliedCouponCode = Convert.ToString(r["Code"]) ?? code; couponDiscount = discount;
        ...
        private void RemoveCoupon(bool showMessage = true)
        {
            ...
            appliedCouponCode = ""; couponDiscount = 0m; txtCoupon.Clear();
            lblCouponStatus.Text = "No coupon applied."; lblCouponStatus.ForeColor = Color.DimGray;
            RecalculateTotals();
        ...
        private void UpdatePaymentFields()
        {
            ...
            txtTransaction.Enabled = online; txtTransaction.Visible = online;
        ...
        private void BtnPay_Click(object sender, EventArgs e)
        {
            ...
            if (cboPayment.SelectedItem == null) { ShowError("Choose a payment option to continue."); return; }
            string method = cboPayment.SelectedItem.ToString(); bool online = method != "Pay at Hotel"; string transactionId = txtTransaction.Text.Trim();
            if (online && string.IsNullOrWhiteSpace(transactionId)) { ShowError("Please enter the transaction ID for your online payment."); txtTransaction.Focus(); return; }
            decimal roomTotal = 0, serviceTotal = 0, offerDiscount = 0;
            foreach (var l in lines) { roomTotal += l.RoomSubtotal; serviceTotal += l.ServiceAmount; offerDiscount += l.DiscountAmount; }
            decimal baseTotal = Math.Max(0m, roomTotal + serviceTotal - offerDiscount);
            ...
            catch (Exception ex) { btnConfirm.Enabled = true; ShowError(ex.Message); }
        }
        ...
        {
            bottomPanel.Visible = true; bottomPanel.Height = 0;
            var box = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 2, RowCount = 7, Padding = new Padding(16), BackColor = Color.White, BorderStyle = BorderStyle.FixedSingle };
            box.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 70)); box.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 30));
            box.Controls.Add(new Label { Text = $"BOOKING REQUEST SUBMITTED  •  #{id}", Dock = DockStyle.Fill, Font = new Font("Segoe UI", 14F, FontStyle.Bold), ForeColor = UIHelper.NavyHeader }, 0, 0); box.SetColumnSpan(box.GetControlFromPosition(0, 0), 2);
            box.Controls.Add(new Label { Text = pendingApproval ? $"Guest: {Session.FullName}\nPayment option: {method}\nTransaction ID: {transactionId}\nCoupon: {(string.IsNullOrWhiteSpace(couponCode) ? "None" : couponCode)}\nStatus: Pending transaction validation\n\nYour cart has been cleared. After the hotel validates your transaction ID, the booking will be confirmed and the dashboard will notify you.\nConfirmation email will be sent to your registered email after validation." : $"Guest: {Session.FullName}\nPayment option: {method}\nCoupon: {(string.IsNullOrWhiteSpace(couponCode) ? "None" : couponCode)}\nStatus: Confirmed\n\nYour Pay at Hotel booking is confirmed. Your cart has been cleared.", Dock = DockStyle.Fill }, 0, 1); box.SetColumnSpan(box.GetControlFromPosition(0, 1), 2);
            box.Controls.Add(new Label { Text = "Room amount", Dock = DockStyle.Fill }, 0, 2); box.Controls.Add(new Label { Text = $"${room:N2}", Dock = DockStyle.Fill, TextAlign = ContentAlignment.MiddleRight }, 1, 2);
            box.Controls.Add(new Label { Text = "Services", Dock = DockStyle.Fill }, 0, 3); box.Controls.Add(new Label { Text = $"${services:N2}", Dock = DockStyle.Fill, TextAlign = ContentAlignment.MiddleRight }, 1, 3);
            box.Controls.Add(new Label { Text = $"Discount {(string.IsNullOrWhiteSpace(couponCode) ? "" : "(coupon " + couponCode + ")")}", Dock = DockStyle.Fill }, 0, 4); box.Controls.Add(new Label { Text = $"-${discount:N2}", Dock = DockStyle.Fill, TextAlign = ContentAlignment.MiddleRight }, 1, 4);
            box.Controls.Add(new Label { Text = "Grand total", Dock = DockStyle.Fill, Font = new Font("Segoe UI", 11F, FontStyle.Bold) }, 0, 5); box.Controls.Add(new Label { Text = $"${total:N2}", Dock = DockStyle.Fill, TextAlign = ContentAlignment.MiddleRight, Font = new Font("Segoe UI", 12F, FontStyle.Bold), ForeColor = UIHelper.CustomerColor }, 1, 5);
            var done = UIHelper.MakeButton("View My Bookings", UIHelper.CustomerColor, 200, 36); done.Anchor = AnchorStyles.None; done.Click += (s, e) => { new BookingHistoryForm().Show(); Close(); }; box.Controls.Add(done, 0, 6); box.SetColumnSpan(done, 2);
            bottomPanel.Controls.Add(box);
            bottomPanel.Height = 260;
        }

        private void ShowError(string text) { lblError.Text = text; lblError.Visible = true; }
    }
}
